...
def calculate_average_inter_agent_PLV(phase_matrices, window_length, window_step):
    # calculate windowed PLV
    window_start = 0
    window_end = window_start + window_length
    ...
    while (window_start + window_length) < simulation_length:
        ...
        for a_i in range(n_agents):
            for a_j in range(a_i, len(phase_matrices)):
                ...
                for i in range(_n_oscillators):
                    ...
                    if i == 4: # if you have a fifth oscillator
                        plv_5 += np.abs(np.mean(np.exp(1j *(phase_matrices[a_i][i, window_start:window_end] - phase_matrices[a_j][i, window_start:window_end]))))
                    counter += 1 
        window_start += window_step
        window_end += window_step
...
window_length = fs
window_step = fs/10
plv_in_time, interval_times, mean_plv, plv_5_in_time, mean_plv_5 = calculate_average_inter_agent_PLV(phase_matrices, window_length, window_step)
