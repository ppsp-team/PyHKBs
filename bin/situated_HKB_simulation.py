...
from bin.agent import Agent, Agent_single_oscillator
import time
...
stimulus_position = torch.tensor([-100., 0.]) # [m, m]
...
frequencies = torch.tensor([5.0, 5.0]) # Hertz
...
agent_position = torch.tensor([0., -100.]) # [m, m]
agent_orientation = torch.as_tensor([-np.pi/2]) # rad
